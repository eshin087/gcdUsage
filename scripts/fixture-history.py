...
root=Path(os.environ.get('GCD_FIXTURE_DIR','.local-test/v04')).resolve();data=root/'runtime';codex=root/'codex';claude=root/'claude'
...
for project in ['Orbit Console','Nebula Tools']:
 ...
 for i in range(12):
  turn=str(uuid.uuid4());t=now-1200+i*60;body=['Improve the search experience and keyboard navigation','Review the cache boundaries and performance budget','<in-app-browser-context source="ambient-ui-state"><div>Internal browser source that must never appear</div></in-app-browser-context>Explain this function clearly','<send_user_message_question_reply>[{"question":"Preferred interval?","answer":"Last hour"}]</send_user_message_question_reply>','Display <script> text safely without executing markup'][i%5]

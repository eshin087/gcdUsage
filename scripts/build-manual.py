...
def header(number,label,title,subtitle):
 ...
 c.setFillColor(muted);c.setFont('Helvetica',9);c.drawRightString(564,742,'QUICK MANUAL  /  v0.5.4')
...
y=section('1. Install the matching package','Install a manually validated package for your platform. Windows installs for your user; macOS packages require a Mac build. Check the repository validation notes for the versions available on each platform. Personal preview builds are unsigned or ad-hoc signed; follow the operating system\'s first-launch approval only for a package you trust.',y)
y=section('2. Connect your coding tools','Install and sign in to Claude Code and Codex first. The app reuses saved sign-ins after app and computer restarts. GCD Usage asks a current Claude Code helper to renew expired access using its saved refresh token; Claude Code owns credential storage. Manual logout, revoked access or a missing refresh token still requires sign-in. Organization SSO depends on the account.',y)
...
y=section('4. Make it yours','The default is cream-on-charcoal, percentage left, larger text and a one-hour dock interval. Saving a theme changes the dashboard and native Windows surfaces together. Dock size is independent from dashboard text. Closing the dashboard keeps the app running; Quit ends the app.',y)
...
y=section('Read the allowance cards','Claude has five-hour, weekly and labeled Fable allowances; Codex shows weekly allowance. Missing values remain unavailable; stale readings are marked. Reset timing uses muted red. Recent usage shows the change over your selected duration: 91% to 87% left means 4% used. Incomplete coverage is labeled observed. Never add different allowance windows together.',y)
y=section('Change time directly from the dock','The token card and right-click menu select a rolling duration. The same duration controls recent percentage consumption on the dock, Overview and Model advice. Choose 30 minutes, 1 hour or another preset; Custom accepts whole minutes, hours or days, from 1 minute through 30 days. Apply saves it; Cancel keeps the previous value.',y)
...
y=section('Resize, hide and restore','Drag any unlocked corner to resize the dock, keeping its proportions, or use the 80-160% size control. Drag the middle to move it. Click Hide to hide the dock; it stays hidden after restarting. Double-click the tray icon or right-click it and choose Show dock to restore it. Right-click the tray for Open dashboard. Lock dock position also locks resizing.',y)
...
y=note('Patterns and allowance forecasts','Model advice charts 7, 30 or 90 days of activity, model mix, cache reuse and busy hours. The allowance forecast uses provider percentage changes, with at least four readings spanning 30 minutes and no gap over 15 minutes. It pauses for stale readings or resets and projects only until reset or exhaustion. Per-model advice needs enough completed history. All calculations run locally; activity does not measure model quality, and your model never changes automatically.',y)
finish(y)
...
y=section('Security and privacy','The dashboard uses narrow commands, restricted navigation and bounded reads. Credentials stay with provider storage, outside history and sync. Renewal uses a restricted helper environment and deadline. Local account/admin malware remains outside this boundary; a review cannot guarantee the absence of every vulnerability.',y)

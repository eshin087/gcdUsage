...
def header(number,label,title,subtitle):
 ...
 c.setFillColor(muted);c.setFont('Helvetica',9);c.drawRightString(564,742,'QUICK MANUAL  /  v0.5.1')
...
y=section('4. Make it yours','The default is a black Matrix-style theme, percentage left, larger text, an unlocked Windows dock, and a one-hour dock activity interval. Adjust theme, text size and launch-at-login in Settings. Dock size has its own 80-160% slider and right-click presets; its default is 100%. Closing the dashboard keeps the native meters running; Quit ends the app.',y)
...
y=header(2,'Glance and control','The dock and the dashboard','Windows uses a slim, movable terminal-style dock. macOS uses native menu-bar meters; the Windows hover and duration controls do not apply to macOS.')
...
y=section('Hover, scroll, click','Hover a usage card for the latest ten recorded prompts for that provider. The token card combines providers. Rows show project/chat context, a short prompt preview, model/reasoning, time and measured lifetime tokens. Scroll if the screen cannot fit all ten. Click a row to open captured details.',y)

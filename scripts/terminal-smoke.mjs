...
try {
 await page.goto('http://127.0.0.1:1420/?preview=1');await page.locator('.quota-card').first().waitFor();await page.getByText('Updating statistics…',{exact:true}).waitFor({state:'hidden'});
 ...
 assert.equal(await page.getByLabel('Remaining requests unavailable').textContent(),'â€”');
 ...
 await writeFile('.local-test/v05/ui-report.json',JSON.stringify({singleLineHistory:true,manualBrowserUIRemoved:true,proRemainingUnknown:true,responsiveWidths:[900,1000,1280],errors},null,2));

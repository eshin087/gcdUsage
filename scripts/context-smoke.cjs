...
(async()=>{const browser=await chromium.connectOverCDP('http://127.0.0.1:9223');try{
 ...
 const history=await invoke('get_history',{filter:{limit:50}});assert.equal(history.total,36);assert.ok(history.items.every(r=>r.prompt.project));assert.ok(history.items.every(r=>!r.prompt.preview.includes('<send_user_message')));
 ...
 assert.deepEqual(errors,[]);const report={prompts:history.total,contextLabels:true,wrapperCleanup:true,detailNavigation:true,untrustedLinksRejected:true,defaultFont:18,errors};await fs.writeFile('.local-test/v04/context-report.json',JSON.stringify(report,null,2));console.log(JSON.stringify(report));
